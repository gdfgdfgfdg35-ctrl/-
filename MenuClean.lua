...
if Environment.__CleanEspUnload then
    Environment.__CleanEspUnload()
end
...
local Config = {
    ...
    VisibilityRefresh = 0.12,

    BoxVisibleColor = Color3.fromRGB(70, 220, 255),
    BoxHiddenColor = Color3.fromRGB(255, 85, 100),
    SkeletonVisibleColor = Color3.fromRGB(90, 235, 255),
    SkeletonHiddenColor = Color3.fromRGB(255, 100, 115),
    NameVisibleColor = Color3.fromRGB(235, 245, 255),
    NameHiddenColor = Color3.fromRGB(255, 135, 145),
    DistanceVisibleColor = Color3.fromRGB(190, 225, 255),
    DistanceHiddenColor = Color3.fromRGB(255, 150, 155),
    TracerVisibleColor = Color3.fromRGB(70, 220, 255),
    TracerHiddenColor = Color3.fromRGB(255, 85, 100),
    HealthHighColor = Color3.fromRGB(80, 255, 100),
    HealthLowColor = Color3.fromRGB(255, 60, 70),
    OutlineColor = Color3.fromRGB(8, 10, 14),
}

local SkeletonCornerSteps = 3
local SkeletonCornerRoundness = 0.22
local MaximumSkeletonSegments = 44

local R6SkeletonPaths = {
    { "Head", "Torso" },
    { "Torso", "Left Arm" },
    { "Torso", "Right Arm" },
    { "Torso", "Left Leg" },
...
LabelGui.Name = "__CleanEspLabels"
LabelGui.IgnoreGuiInset = true
LabelGui.ResetOnSpawn = false
LabelGui.DisplayOrder = 10000
LabelGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling

local GuiParent = gethui and gethui() or game:GetService("CoreGui")
if protectgui then
    pcall(protectgui, LabelGui)
end
LabelGui.Parent = GuiParent
...
local function NewLabel(TextSize, Color)
    local Label = Instance.new("TextLabel")
    Label.Name = "EspLabel"
    Label.AnchorPoint = Vector2.new(0.5, 0.5)
    Label.BackgroundTransparency = 1
    Label.BorderSizePixel = 0
    Label.Size = UDim2.fromOffset(320, TextSize + 8)
...
local function HideEntry(Entry)
    for _, Object in Entry.Objects do
...
local function RemoveEntry(Player)
    ...
    for Humanoid, OriginalDisplayType in Entry.HiddenHumanoids or {} do
        pcall(function()
            Humanoid.DisplayDistanceType = OriginalDisplayType
        end)
    end
    table.clear(Entry.HiddenHumanoids)
...
local function CreateEntry(Player)
    ...
    local Entry = {
        ...
        Box = NewDrawing("Square", {
            Filled = false,
            Thickness = 1,
            Color = Config.BoxVisibleColor,
            Transparency = 1,
            Visible = false,
        }),
        Name = NewLabel(16, Config.NameVisibleColor),
        Distance = NewLabel(14, Config.DistanceVisibleColor),
        ...
        Tracer = NewDrawing("Line", {
            Thickness = 1,
            Color = Config.TracerVisibleColor,
            Transparency = 1,
            Visible = false,
        }),
    }

    Entry.SkeletonOutlines = {}
    Entry.Skeletons = {}
    for Index = 1, MaximumSkeletonSegments do
        Entry.SkeletonOutlines[Index] = NewDrawing("Line", {
            Thickness = 3,
            Color = Config.OutlineColor,
            Transparency = 1,
            Visible = false,
        })
        Entry.Skeletons[Index] = NewDrawing("Line", {
            Thickness = 1,
            Color = Config.SkeletonVisibleColor,
            Transparency = 1,
            Visible = false,
        })
    end
    ...
    for Index = 1, MaximumSkeletonSegments do
        table.insert(Entry.Objects, Entry.SkeletonOutlines[Index])
        table.insert(Entry.Objects, Entry.Skeletons[Index])
    end
    ...
    local function BindCharacter(Character)
        ...
        task.spawn(function()
            ...
            if not Humanoid
                or Entries[Player] ~= Entry
                or Player.Character ~= Character then
                return
            end

            if Entry.HiddenHumanoids[Humanoid] == nil then
                Entry.HiddenHumanoids[Humanoid] = Humanoid.DisplayDistanceType
            end
            ...
            table.insert(Entry.Connections, DeathConnection)
        end)
    end

    table.insert(Entry.Connections, Player.CharacterAdded:Connect(BindCharacter))
...
local function GetScreenBounds(Camera, Character)
    ...
    -- Project every part independently. This stays tight around animated limbs
    -- and still includes hats/accessories instead of using one coarse 3D box.
            ...
            for X = -1, 1, 2 do
                for Y = -1, 1, 2 do
                    for Z = -1, 1, 2 do
                        ...
                        local Screen = Camera:WorldToViewportPoint(Corner)
    ...
    return Vector2.new(MinimumX, MinimumY), Vector2.new(Width, Height)
end

local function UpdateSkeleton(Entry, Character, Humanoid, Camera, Color)
    local Definition = Humanoid.RigType == Enum.HumanoidRigType.R6
        and R6SkeletonPaths
        or R15SkeletonPaths
    local SegmentIndex = 1

    local function DrawSegment(From, To)
        if SegmentIndex > MaximumSkeletonSegments then
            return
        end
        local Outline = Entry.SkeletonOutlines[SegmentIndex]
        local Bone = Entry.Skeletons[SegmentIndex]
        Outline.From = From
        Outline.To = To
    ...
    if Config.Skeletons then
        for _, Path in Definition do
            local Points = {}
            local Valid = true
            for _, PartName in Path do
            ...
            if Valid and #Points >= 2 then
                ...
                for PointIndex = 2, #Points - 1 do
                    ...
                    local Joint = Points[PointIndex]
                    local Following = Points[PointIndex + 1]
                    local CurveStart = Joint:Lerp(Previous, SkeletonCornerRoundness)
                    local CurveEnd = Joint:Lerp(Following, SkeletonCornerRoundness)

                    DrawSegment(Current, CurveStart)
                    local LastPoint = CurveStart
                    for Step = 1, SkeletonCornerSteps do
                        local T = Step / SkeletonCornerSteps
                        local Inverse = 1 - T
                        local CurvePoint = CurveStart * (Inverse * Inverse)
                            + Joint * (2 * Inverse * T)
                            + CurveEnd * (T * T)
                        DrawSegment(LastPoint, CurvePoint)
                        LastPoint = CurvePoint
                    end
                    Current = CurveEnd
                end
                DrawSegment(Current, Points[#Points])
            end
        end
    end

    for Index = SegmentIndex, MaximumSkeletonSegments do
        Entry.SkeletonOutlines[Index].Visible = false
        Entry.Skeletons[Index].Visible = false
    end
end

local function IsCharacterVisible(Camera, Character)
    ...
    for _, Target in Targets do
        if Target and Target:IsA("BasePart") then
            local Result = workspace:Raycast(Origin, Target.Position - Origin, Parameters)
...
local function UpdateEntry(Player, Entry, Camera)
    if not Config.Enabled or Player == LocalPlayer then
        HideEntry(Entry)
        return
    end

    local Friendly = IsFriendly(Player)
    if Config.TeamCheck and Friendly then
        HideEntry(Entry)
        return
    end
    ...
    local Position, Size = GetScreenBounds(Camera, Character)
    ...
    elseif not Entry.LastVisibilityCheck
        or Now - Entry.LastVisibilityCheck >= Config.VisibilityRefresh then
        Entry.LastVisibilityCheck = Now
        Entry.IsVisible = IsCharacterVisible(Camera, Character)
    end

    local IsVisible = Entry.IsVisible ~= false
    local BoxColor = IsVisible and Config.BoxVisibleColor or Config.BoxHiddenColor
    local SkeletonColor = IsVisible and Config.SkeletonVisibleColor or Config.SkeletonHiddenColor
    local NameColor = IsVisible and Config.NameVisibleColor or Config.NameHiddenColor
    local DistanceColor = IsVisible and Config.DistanceVisibleColor or Config.DistanceHiddenColor
    local TracerColor = IsVisible and Config.TracerVisibleColor or Config.TracerHiddenColor
    local CenterX = Position.X + Size.X * 0.5

    Entry.BoxOutline.Position = Position
    Entry.BoxOutline.Size = Size
    Entry.BoxOutline.Visible = Config.Boxes
    ...
    Entry.Name.TextColor3 = NameColor
    Entry.Name.Position = UDim2.fromOffset(CenterX, Position.Y - 12)
    Entry.Name.Visible = Config.Names

    Entry.Distance.Text = string.format("[%d studs]", math.floor(Distance + 0.5))
    Entry.Distance.TextColor3 = DistanceColor
    Entry.Distance.Position = UDim2.fromOffset(CenterX, Position.Y + Size.Y + 11)
    Entry.Distance.Visible = Config.Distance

    local HealthRatio = math.clamp(Humanoid.Health / math.max(Humanoid.MaxHealth, 1), 0, 1)
    local BarHeight = Size.Y * HealthRatio
    Entry.HealthOutline.Position = Vector2.new(Position.X - 7, Position.Y - 1)
    Entry.HealthOutline.Size = Vector2.new(4, Size.Y + 2)
    Entry.HealthOutline.Visible = Config.HealthBars

    Entry.Health.Position = Vector2.new(Position.X - 6, Position.Y + Size.Y - BarHeight)
    Entry.Health.Size = Vector2.new(2, BarHeight)
    Entry.Health.Color = Config.HealthLowColor:Lerp(Config.HealthHighColor, HealthRatio)
    Entry.Health.Visible = Config.HealthBars

    local Viewport = Camera.ViewportSize
    local TracerStart = Vector2.new(Viewport.X * 0.5, Viewport.Y - 2)
    local TracerEnd = Vector2.new(CenterX, Position.Y + Size.Y)
    Entry.TracerOutline.From = TracerStart
    Entry.TracerOutline.To = TracerEnd
    Entry.TracerOutline.Visible = Config.Tracers
    Entry.Tracer.From = TracerStart
    Entry.Tracer.To = TracerEnd
    Entry.Tracer.Color = TracerColor
    Entry.Tracer.Visible = Config.Tracers

    UpdateSkeleton(Entry, Character, Humanoid, Camera, SkeletonColor)
end
...
Connect(RunService.RenderStepped, function()
    ...
    for Player, Entry in Entries do
        UpdateEntry(Player, Entry, Camera)
    end
end)

-- Reconcile occasionally as a safety net for injection during a join/leave or
-- an executor event missed while its environment was being replaced.
local ReconcileAccumulator = 0
Connect(RunService.Heartbeat, function(DeltaTime)
    ReconcileAccumulator += DeltaTime
    if ReconcileAccumulator < 1 then
        return
    end
    ...
    for _, Player in Players:GetPlayers() do
        Present[Player] = true
        if Player ~= LocalPlayer and not Entries[Player] then
            CreateEntry(Player)
        end
    end
...
Connect(UserInputService.InputBegan, function(Input)
    if Input.KeyCode == Config.UnloadKey then
        local Unload = Environment.__CleanEspUnload
        if Unload then
            Unload()
        end
    end
...
Environment.__CleanEspUnload = function()
    if not Running then
        return
    end
    Running = false

    -- The menu owns internal connections which it releases from its unload
    -- key handler. Send that key when ESP is unloaded programmatically too.
    if MenuAPI then
        pcall(function()
        ...
        pcall(function()
            local MenuGui = MenuAPI.GetScreenGui and MenuAPI.GetScreenGui()
            if MenuGui then
                MenuGui:Destroy()
            end
        end)
    ...
    Environment.__CleanEspUnload = nil
end

local EMBEDDED_MENU_SOURCE = [====[
local MenuLib = {}
...
function MenuLib:Init(config)
    ...
    API.AddColorPicker = function(tabName, label, callback, defaultColor, side, compact)
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            local title = lbl(header, label .. "  •  COLOR V2", UDim2.new(1, -92, 1, 0), UDim2.fromOffset(14, 0), 12, C.TEXT, Enum.Font.GothamBold)
            title.ZIndex = 1002
...
local function LoadMenu()
    local Success, Result = pcall(function()
        local Factory, CompileError = loadstring(EMBEDDED_MENU_SOURCE)
        assert(Factory, "Bundled MenuClean did not compile: " .. tostring(CompileError))
        local Library = Factory()
        assert(Library and Library.Init, "Bundled MenuClean returned an invalid library")
        print("[Clean ESP] Loaded bundled COLOR V2 menu (no GitHub/local fallback)")
        return Library:Init({
            Title = "Clean ESP • COLOR V2",
        })
    end)

    if not Success or not Result then
        warn("MenuClean failed to load:", Result)
        return
    end

    MenuAPI = Result
    _G._MenuToggleKey = Enum.KeyCode.Insert
    _G._UnloadKey = Enum.KeyCode.End

    MenuAPI.AddToggle("Visuals", "ESP Enabled", function(Value)
        Config.Enabled = Value
    end, Config.Enabled, "left")

    local function AddPairedColorToggle(Label, ToggleKey, VisibleKey, HiddenKey, Side)
        return MenuAPI.AddColorToggle(
            "Visuals",
            Label,
            function(Value) Config[ToggleKey] = Value end,
            Config[ToggleKey],
            function(Color) if typeof(Color) == "Color3" then Config[VisibleKey] = Color end end,
            Config[VisibleKey],
            function(Color) if typeof(Color) == "Color3" then Config[HiddenKey] = Color end end,
            Config[HiddenKey],
            Side
        )
    end

    if MenuAPI.AddColorToggle then
        AddPairedColorToggle("Boxes", "Boxes", "BoxVisibleColor", "BoxHiddenColor", "left")
        AddPairedColorToggle("Smooth Skeleton", "Skeletons", "SkeletonVisibleColor", "SkeletonHiddenColor", "left")
        AddPairedColorToggle("Names", "Names", "NameVisibleColor", "NameHiddenColor", "left")
        AddPairedColorToggle("Distance", "Distance", "DistanceVisibleColor", "DistanceHiddenColor", "left")
        AddPairedColorToggle("Health Bars", "HealthBars", "HealthHighColor", "HealthLowColor", "right")
        AddPairedColorToggle("Tracers", "Tracers", "TracerVisibleColor", "TracerHiddenColor", "right")
    else
        warn("MenuClean local compact-color API is missing; update MenuClean.lua")
        MenuAPI.AddToggle("Visuals", "Boxes", function(Value) Config.Boxes = Value end, Config.Boxes, "left")
        MenuAPI.AddToggle("Visuals", "Smooth Skeleton", function(Value) Config.Skeletons = Value end, Config.Skeletons, "left")
        MenuAPI.AddToggle("Visuals", "Names", function(Value) Config.Names = Value end, Config.Names, "left")
        MenuAPI.AddToggle("Visuals", "Distance", function(Value) Config.Distance = Value end, Config.Distance, "left")
        MenuAPI.AddToggle("Visuals", "Health Bars", function(Value) Config.HealthBars = Value end, Config.HealthBars, "right")
        MenuAPI.AddToggle("Visuals", "Tracers", function(Value) Config.Tracers = Value end, Config.Tracers, "right")
    end

    MenuAPI.AddToggle("Visuals", "Team Check", function(Value)
        Config.TeamCheck = Value
    end, Config.TeamCheck, "right")

    MenuAPI.AddToggle("Visuals", "Visibility Check", function(Value)
    ...
    end, Config.VisibilityCheck, "right")

    MenuAPI.AddSlider("Visuals", "Maximum Distance", 100, 5000, function(Value)
        Config.MaxDistance = Value
    end, Config.MaxDistance, "right")
